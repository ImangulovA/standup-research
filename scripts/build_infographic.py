...
:root{
  ...
  --shadow-glow-pink:0 0 12px rgba(255,0,60,.35); --shadow-glow-cyan-hot:0 0 12px rgba(0,240,255,.35);
...
.chip{display:inline-flex;align-items:center;gap:8px;padding:8px 15px;border:1px solid var(--cyan-30);
  background:var(--bg-input);font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:var(--tracking-wider);
  color:var(--text-secondary);transition:all .2s ease}
.chip .d{width:8px;height:8px;border-radius:50%;animation:pulse 1.6s ease infinite}
.chip.cyan{border-color:var(--cyan);color:var(--cyan);background:var(--cyan-08);box-shadow:var(--shadow-glow-cyan-hot)}
.chip.cyan:hover{background:var(--cyan-10);box-shadow:var(--shadow-hover-cyan)}
.chip.pink{border-color:var(--pink);color:var(--pink);background:var(--pink-08);box-shadow:var(--shadow-glow-pink)}
.chip.pink:hover{background:rgba(255,0,60,.12);box-shadow:0 0 20px rgba(255,0,60,.45)}
.chip.v{border-color:var(--border-medium);color:var(--text-faint);background:none;pointer-events:none}
...
footer a.fl-cyan{color:var(--cyan);font-weight:700} footer a.fl-cyan:hover{text-shadow:0 0 8px var(--cyan)}
footer a.fl-pink{color:var(--pink);font-weight:700} footer a.fl-pink:hover{text-shadow:0 0 8px var(--pink)}
...
<div class="wrap">
  <div class="topbar fade">
    ...
    <div class="chips">
      <a class="chip cyan" href="https://t.me/newezha" target="_blank" rel="noopener"><span class="d" style="background:var(--cyan)"></span>Невежда</a>
      <a class="chip pink" href="https://t.me/Chappellepozvonit" target="_blank" rel="noopener"><span class="d" style="background:var(--pink)"></span>Шаппелл позвонит</a>
  ...
  <footer>
    Проект каналов <a class="fl-cyan" href="https://t.me/newezha" target="_blank" rel="noopener">Невежда</a> и
    <a class="fl-pink" href="https://t.me/Chappellepozvonit" target="_blank" rel="noopener">Шаппелл позвонит</a>.<br>
    Каталог концертов: <a href="https://smeshno.xyz/youtube/full/" target="_blank" rel="noopener">смешно.xyz</a> (Илья Якямсев).
...
function drawChart(hl){
  ...
  const W=svg.clientWidth||960,padL=14,padR=14,padB=34,padT=44;
  const lo=0,hi=5000;
  ...
  for(const l of LAND){if(l.v<lo||l.v>hi)continue;
    s+=`<g class="land"><line x1="${x(l.v)}" y1="${padT-8}" x2="${x(l.v)}" y2="${H-padB}"/>`+
       `<text transform="translate(${x(l.v)-4},${H-padB-8}) rotate(-90)" text-anchor="start">${fmt(l.v)} · ${l.t}</text></g>`;}
